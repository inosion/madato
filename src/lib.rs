#![feature(proc_macro, wasm_custom_section, wasm_import_module, slice_concat_ext, iterator_flatten)]
...
pub fn mk_header(heading_data: &[(&str, usize)]) -> String {
  ...
  let heading: String = heading_data.iter().fold(String::from("|"), |res, h| format!("{}{: ^width$}|",res,h.0,width = h.1) );
  let dashed: String  = heading_data.iter().fold(String::from("|"), |res, h| format!("{}{:-^width$}|",res,"-",width = h.1) );
...
/// 
pub fn mk_data<T : ToString + ?Sized>(heading_data: &[(&str, usize)], data: &[BTreeMap<&str, &T>]) -> String {

  let ret: Vec<String> = data.iter().map(|hm| {
...
pub fn mk_table<T : ToString + ?Sized>(headings: &[&str], data: &[BTreeMap<&str,  &T>]) -> String {
  ...
  let heading_data: Vec<(&str, usize)> = 
    headings.iter()
            .map( |h| {  
                          (*h,
                           data.iter()
                               .fold(h.len(), | max, hm |  // how to return a 
                                   cmp::max(max,
                                     match hm.get(h)  {
  ...
  format!("{}\n{}",mk_header(&heading_data), mk_data(&heading_data, data))

}

#[test]
fn can_make_table_all_cols() {
  let tbl_md = mk_table_all_cols(
                    &vec![
                         treemap!["foo" => "ggg", "bar" => "fred", "nop" => "no"],
                         treemap!["foo" => "seventy", "bar" => "barry", "nop" => "no"],
                         treemap!["bar" => "col has no foo"]
...
fn can_yaml_to_md() {
  ...
  let expected = "
    ...
    ||190x|       |  that   |        nice        |
    ||100 | ta da |  this   |someother value here|".strip_margin();
...
pub fn mk_md_table_from_yaml(yaml: &str) -> String{
  ...
  let str_btm = deserialized_map.iter().map(|btree|
        btree.iter().map(|(x, y)| (x.as_str(), y.as_str())).collect::<BTreeMap<&str, &str>>()
    ).collect::<Vec<_>>();
...
fn can_yaml_to_md_with_headings() {
  ...
  let expected = "
    ...
    ||  that   |        nice        |       |
    ||  this   |someother value here| ta da |".strip_margin();
